...
{
    std::visit(common::overloaded{[&](const auto &m)
                                  {
                                      os << m;
                                  }},
               msg);
...
std::string getName(const BitTorrentMessage &msg)
{
    return std::visit(
        common::overloaded{
            [](const Choke &) -> std::string
            {
                return "Choke";
            },
            [](const UnChoke &) -> std::string
            {
                return "UnChoke";
            },
            [](const Interested &) -> std::string
            {
                return "Interested";
            },
            [](const NotInterested &) -> std::string
            {
                return "NotInterested";
            },
            [](const Have &) -> std::string
            {
                return "Have";
            },
            [](const Bitfield &) -> std::string
            {
                return "Bitfield";
            },
            [](const Request &r)
            {
                return "Request{" + std::to_string(r.getReqIndex()) + "," +
                       std::to_string(r.getReqBegin()) + "," + std::to_string(r.getReqLength()) +
                       "}";
            },
            [](const Piece &) -> std::string
            {
                return "Piece";
            },
            [](const Cancel &) -> std::string
            {
                return "Cancel";
            },
            ...
            [](const KeepAlive &) -> std::string
            {
                return "KeepAlive";
            },
            [](const HandShake &) -> std::string
            {
                return "HandShake";
            },
            [](const SerializeError &) -> std::string
            {
...
bool operator==(const BitTorrentMessage &lhs, const BitTorrentMessage &rhs)
{
    return lhs.index() == rhs.index() &&
           std::visit(common::overloaded{[&](const auto &msg)
                                         {
...
HandShake::HandShake(const std::string &pstr, const std::array<char, 8> &reserved,
                     const std::array<char, 20> &infoHash, const std::array<char, 20> &peerId)
    : mPstr(pstr), mReserved(std::move(reserved)), mInfoHash(std::move(infoHash)),
...
{
    return rq1.getReqIndex() == rq2.getReqIndex() && rq1.getReqBegin() == rq2.getReqBegin() &&
...
{
    return p1.getPieceIndex() == p2.getPieceIndex() && p1.getPieceBegin() == p2.getPieceBegin() &&
...
Cancel::Cancel(uint32_t index, uint32_t begin, uint32_t len)
    : mIndex(index), mBegin(begin), mLen(len)
{
}
...
const std::string& SerializeError::getError() const
{
    return mError;
}
